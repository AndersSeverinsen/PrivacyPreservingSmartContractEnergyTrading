...
for num_of in range(MIN, MAX+1, JUMP):
    for repetition in range(REPETITIONS):
        ...
        command = f'cargo pbc transaction deploy --gas=2500000 {PATH}{FILE_NAME}.pbc --privatekey={private_key}'
        ...
        for i in range(NUM_OF_SELL_ORDERS):
            ...
            order_struct = "{ " + f'{i} [ {" ".join(map(str, random_numbers))} ]' + " }"
            command = f'cargo pbc transaction action {contract_address} input_sell_order {order_struct} --gas 60000 --privatekey={private_key}'
        ...
        for i in range(NUM_OF_BUY_ORDERS):
            ...
            order_struct = "{ " + f'{i+NUM_OF_SELL_ORDERS} [ {" ".join(map(str, random_numbers))} ]' + " }"
            command = f'cargo pbc transaction action {contract_address} input_buy_order {order_struct} --gas 60000 --privatekey={private_key}'
